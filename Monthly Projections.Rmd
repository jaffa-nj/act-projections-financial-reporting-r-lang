...
proj.env$MonthlyProjections <- expand.grid(
  Treaty = proj.env$treaty_positions$ConcatenatedKey,
  Month = proj.env$date_vector
) %>%
  left_join(proj.env$treaty_positions, by = c("Treaty" = "ConcatenatedKey")) %>%
  left_join((proj.env$vel_df3 %>% mutate(MONTH = lubridate::ym(MONTH))), by = c("Treaty" = "KEY", "Month" = "MONTH")) %>%
  rename(
    Program_Name = Name,
    Carrier = Carrier,
