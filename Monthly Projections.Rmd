...
proj.env$MonthlyProjections <- proj.env$MonthlyProjections %>%
  mutate(
    WP_Monthly = datareplace(WP_Monthly, WP_Monthly_Data), # Doing this replacement before calculating EP means that EP won't be based on pricing data
    # Amount of Written Premium that will runoff to in next treaty (LOD only)
    WP_Runoff = ifelse(
      RA_or_LOD == "LOD",
...
proj.env$MonthlyProjections <- proj.env$MonthlyProjections %>%
  ...
  mutate(
    ...
    # DUEPR_Balance = cumsum(WP_Runoff)
  ) %>%
  ungroup() %>%
