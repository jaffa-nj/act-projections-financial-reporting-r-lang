...
months_between <- function(EFF, CAL) {
  num_months <- months(as.integer((year(CAL) - year(EFF)) * 12 + (month(CAL) - month(EFF)) + ifelse(day(CAL + 1) == 1, 1, 0)))
  return(num_months)
}
...
proj.env$sqlconn <- dbConnect(odbc::odbc(), 
...
proj.env$treaty_positions <- dbGetQuery(proj.env$sqlconn, paste(readLines("TreatyTable.sql"), collapse = "\n"))

# Close the database connection
dbDisconnect(proj.env$sqlconn)
...
proj.env$MonthlyProjections <- expand.grid(
  ...
  mutate(
    ...
    MonthsNewPolicyInforceAfterTreatyEnds = months(ifelse(
...
proj.env$DEVS_rev <- proj.env$development_factors %>%
  ...
  mutate(LAG_ACCMONTH = months(LAG_ACCMONTH - 1)) # Decrement LAG_ACCMONTH by 1
