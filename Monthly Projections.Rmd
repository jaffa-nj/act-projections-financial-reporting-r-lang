...
months_between <- function(EFF, CAL) {
  num_months <- months(as.integer((year(CAL) - year(EFF)) * 12 + (month(CAL) - month(EFF))))
  dplyr::if_else(day(CAL) == 1 & day(EFF) == as.integer(days_in_month(EFF)), num_months + months(1), num_months)
  return(num_months)
}
...
proj.env$treaty_positions <- read.csv(
  ...
  mutate(
    Effective.Date. = convert_excel_date(Effective.Date.),
    Expiration.Date. = convert_excel_date(Expiration.Date.)
  )
